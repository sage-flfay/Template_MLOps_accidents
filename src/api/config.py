...
# --- LISTE DES MANDATORY CSV RAW FEATURES ---
# --------------------------------------------------
# Black ne s'applique pas grâce à la commande en commentaire (ForMaTting off) pour
# éviter une ribembelle de lignes si affichage sur une ligne > 88 caractères
# fmt: off
CSV_ALL_COLUMNS_2019 = {
    "users": [
...
# A partir de 2019, id_vehicule remplace num_veh pour la jointure. Donc on le
# supprime car il n'apporte aucune information utile pour la prédiction.
# Seul dep, com et hrmn sont des strings.
# Les autres sont des int sauf long et lat qui sont des float
CSV_MANDATORY_COLUMNS = {
    "users": [
        "Num_Acc", "id_vehicule", "place", "catu", "grav", "sexe", "an_nais", "secu1"
    ],
    "veh": ["Num_Acc", "id_vehicule", "catv", "obsm", "motor"],
    "places": ["Num_Acc", "catr", "circ", "surf", "situ", "vma"],
    "caract": [
        "Num_Acc", "jour", "mois", "hrmn", "lum", "dep", "com", "agg", "int",
        "atm", "col", "lat", "long"
    ],
}

# --- LISTE DES FEATURES (ORDRE INDISPENSABLE POUR LE MODÈLE) ---
# Liste affichée dans le WEB du FastApi.
# Sert de référence pour la liste de feature que le modèle doit
# doit prendre pour s'entrainer
FEATURES = [
    "place", "catu", "sexe", "secu1", "year_acc", "victim_age", "catv", "obsm",
    "motor", "catr", "circ", "surf", "situ", "vma", "jour", "mois", "lum", "dep",
    "com", "agg_", "int", "atm", "col", "lat", "long", "hour", "nb_victim",
    "nb_vehicules",
]

# --- LISTE DES FEATURES CRITIQUES - SI NAN, ligne supprimée ---
# NB: donc tous les autres avec une valeur par défaut pour NaN
# A partir de 2019, id_vehicule remplace num_veh pour la jointure. Donc on le
# supprime car il n'apporte aucune information utile pour la prédiction.
FEATURES_CRITICAL = ["Num_Acc", "id_vehicule", "catu", "grav", "an_nais", "catv"]

# --- LABELS POUR L'INTERFACE UTILISATEUR ---
FEATURE_LABELS = [
    "Place de l’accidenté", "Catégorie d’usager", "Sexe de l’usager",
    "Dispositif de sécurité", "Année de l’accident", "Âge de la victime",
    "Catégorie du véhicule", "Obstacle mobile", "Motorisation", "Catégorie de route",
    "Régime de circulation", "État de la surface", "Situation de l’accident",
    "Vitesse maximale autorisée", "Jour du mois", "Mois", "Luminosité", "Département",
    "Code commune", "Agglomération", "Type d’intersection",
    "Conditions atmosphériques", "Type de collision", "Latitude", "Longitude",
    "Heure", "Nombre de victimes", "Nombre de véhicules",
]
# fmt: on
# Black recorrige à partir de maintenant
# --------------------------------------------------
