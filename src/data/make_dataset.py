...
def main():
    ...
    # input_filepath = sys.argv[1] inutile car fait dans le try
    ...
    # Dans le try on a input_filepath = Path(sys.argv[1]),
...
def process_data(
    ...
    # --Replacing names
    # Python 3.12 nouvelle syntaxe (plus explicite) pour éviter les warnings:
    # df_users.grav.replace([1, 2, 3, 4], [1, 3, 4, 2], inplace=True)
    df_users["grav"] = df_users["grav"].replace([1, 2, 3, 4], [1, 3, 4, 2])
    # Préférable d'éviter le inplace=True, alors axis=1 devient inutile
    ...
    df["grav"] = df["grav"].replace([1, 2, 3, 4], [0, 0, 1, 1])
